<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="小吃名" prop="name">
        <el-input
    ...
    <el-table v-loading="loading" :data="foodList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="小吃名" align="center" prop="name" />
      <el-table-column label="价格" align="center" prop="price" :formatter="formatPrice" />
      <el-table-column label="城市" align="center" prop="city" />
      <el-table-column label="小吃图片" align="center" prop="picture" width="100">
        <template slot-scope="scope">
          <image-preview :src="scope.row.picture" :width="50" :height="50"/>
        </template>
      </el-table-column>
      <el-table-column label="餐厅" align="center" prop="restaurant" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="小吃名" prop="name">
          <el-input v-model="form.name" placeholder="请输入小吃名" />
        </el-form-item>
        <el-form-item label="城市" prop="city">
        ...
        <el-form-item label="小吃图片">
          <image-upload v-model="form.picture"/>
        </el-form-item>
        <el-form-item label="餐厅" prop="restaurant">
          <el-input v-model="form.restaurant" placeholder="请输入餐厅(多个餐厅请用,隔开)" />
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除小吃编号为"' + ids + '"的数据项？').then(function() {
        return delFood(ids);
      }).then(() => {
        this.getList();
